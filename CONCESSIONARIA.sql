...
COD_FUNC SERIAL NOT NULL REFERENCES FUNCIONARIO(COD_FUNC) ON DELETE CASCADE,
COD_CONC SERIAL NOT NULL REFERENCES CONCESSIONARIA(COD_CONC) ON DELETE CASCADE);
...
MODELO SERIAL NOT NULL REFERENCES MODELO(COD_MOD) ON DELETE CASCADE,
...
ALTER TABLE CONCESSIONARIA ALTER COLUMN CNPJ TYPE VARCHAR;
									   
drop table FUNCIONARIO,CLIENTE,ENDERECO,CONCESSIONARIA,MODELO,FUNCIONARIO_CONCESSIONARIA,CARRO,ATENDIMENTO,
									   RESERVA_TEST_DRIVE,TEST_DRIVE,VENDA
...
BEGIN
			...
			IF (LENGTH(NEW.CPF)=11) THEN
				IF (TG_OP='UPDATE') THEN
					IF (NEW.NOME != OLD.NOME AND NEW.NOME IN (SELECT NOME FROM FUNCIONARIO)) THEN
						RAISE EXCEPTION 'Nome diferente do anterior ou já existente!';
					END IF;

				ELSE
					IF (NEW.NOME IN (SELECT NOME FROM FUNCIONARIO)) THEN
						RAISE EXCEPTION 'Funcionário com esse nome já existe!';
					END IF;	
				END IF;
				RETURN NEW;
			END IF;
		...
		RAISE EXCEPTION 'Idade menor que 18 anos!';
END;
$$ LANGUAGE PLPGSQL;		
...
BEGIN
		IF (LENGTH(NEW.CPF)=11) THEN
			IF (TG_OP='UPDATE') THEN
				IF (NEW.NOME != OLD.NOME AND NEW.NOME IN (SELECT NOME FROM CLIENTE)) THEN
					RAISE EXCEPTION 'Nome já usado';
				END IF;
...
BEGIN
		IF (LENGTH(NEW.CHASSI)=17) THEN
			IF (NEW.STATUS='Novo' AND NEW.KILOMETRAGEM > 0) THEN
				RAISE EXCEPTION 'Não é possível';
			END IF;
...
BEGIN
		...
		INSERT INTO RESERVA_TEST_DRIVE VALUES(DEFAULT, NOW(), CODIGO_MOD, CODIGO_CLI,
									  CODIGO_FUNC_CONC, HR_COMECO, HR_COMECO + 1, DIA);
...
BEGIN
		...
		INSERT INTO TEST_DRIVE VALUES(DEFAULT, NOW(), CODIGO_MOD, CODIGO_CLI, CODIGO_FUNC_CONC,
									  HR_COMECO, HR_FIM, KILO_INI);
...
BEGIN
		...
		INSERT INTO VENDA VALUES(DEFAULT, NOW(), CODIGO_MOD, CODIGO_CLI, CODIGO_FUNC_CONC, 
								 TIPO, CODIGO_CARRO);
...
CREATE OR REPLACE FUNCTION INSERIR_CONCESSIONARIA(CNPJ VARCHAR(20), NOME_FANTASIA VARCHAR(50), 
							RAZAO_SOCIAL VARCHAR(200), RUA_CONC VARCHAR(50), BAIRRO_CONC VARCHAR(50), 
							NUMERO_CONC INT)	
RETURNS VOID AS $$
DECLARE
	COD_ENDERECO INT;
...
--FUNÇÃO FUNCIONÁRIO	
CREATE OR REPLACE FUNCTION INSERIR_FUNCIONARIO(NOME VARCHAR(50), CPF VARCHAR(20), SEXO TIPOS_DE_SEXOS, IDADE INT)	
RETURNS VOID AS $$
BEGIN
...
--FUNÇÃO CLIENTE	
CREATE OR REPLACE FUNCTION INSERIR_CLIENTE(NOME VARCHAR(50), SEXO TIPOS_DE_SEXOS, CPF VARCHAR(20))	
RETURNS VOID AS $$
BEGIN
	INSERT INTO CLIENTE VALUES(DEFAULT,NOME,SEXO,CPF);
END;
$$ LANGUAGE PLPGSQL;
...
--FUNÇÃO CARRO
CREATE OR REPLACE FUNCTION INSERIR_CARRO(CHASSI VARCHAR(20), KM INT, TIPO TIPO_DE_CARROS, 
...
BEGIN
	...
	SELECT COD_CONC INTO CODIGO_CONC FROM CONCESSIONARIA WHERE NOME_FANTASIA=CONC_NOME;
...
-- FUNCAO FUNC_CONC
CREATE OR REPLACE FUNCTION INSERIR_FUNC_CONC(DATA_DE_ADMISSAO DATE, TURNO TIPOS_DE_TURNO,NOME_FUNC VARCHAR(50),
											NOME_CONC VARCHAR(50))	
RETURNS VOID AS $$
DECLARE 
		CODIGO_FUNC INT;
		CODIGO_CONC INT;
BEGIN
	SELECT COD_FUNC INTO CODIGO_FUNC FROM FUNCIONARIO WHERE NOME=NOME_FUNC;
	SELECT COD_CONC INTO CODIGO_CONC FROM CONCESSIONARIA WHERE NOME_FANTASIA=NOME_CONC;
				   
	INSERT INTO FUNCIONARIO_CONCESSIONARIA VALUES(DEFAULT,DATA_DE_ADMISSAO,TURNO,
												 CODIGO_FUNC, CODIGO_CONC);
																									   
END;
$$ LANGUAGE PLPGSQL;
															  
SELECT(INSERIR_CONCESSIONARIA('12345678910','FANTASIA','FANTASIA LTDA','VEREADOR','JACINTA','45'));
SELECT(INSERIR_MODELO('modelo novo', '2014', 'citroen', 'preto', 30000));
SELECT(INSERIR_CARRO('12345678912345678', 0, 'Venda','Novo', 'modelo novo', 'FANTASIA'));
SELECT(INSERIR_FUNCIONARIO('Joao', '12345678911', 'Masculino', 25));			
SELECT(INSERIR_CLIENTE('Maria', 'Feminino','12345678945'));
